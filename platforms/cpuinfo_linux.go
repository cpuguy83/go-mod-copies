...
func getCPUVariantFromArch(arch string) (string, error) {
	...
	if arch == "aarch64" {
		...
		return "", fmt.Errorf("getCPUVariantFromArch invalid arch: %s, %w", arch, ErrInvalid)
	}
	return variant, nil
}
...
func getCPUVariant() (string, error) {
	...
	if err != nil {
		if errors.Is(err, ErrNotFound) {
			...
			if err != nil {
				return "", fmt.Errorf("failure getting machine architecture: %w", err)
			}

			variant, err = getCPUVariantFromArch(arch)
			if err != nil {
				return "", fmt.Errorf("failure getting CPU variant from machine architecture: %w", err)
			}
		} else {
			return "", fmt.Errorf("failure getting CPU variant: %w", err)
		}
	}
